...
def forecast_split(data, window_size=30, dtp=1, test_days=30):
    ...
    X = []
    y = []
    for i in range(window_size, data.shape[0]-dtp):
        X.append(data.values[i-window_size:i,0])
        y.append(data.values[i:i+dtp,0])
...
class Forecastsimulator():
    ...
    def train_cnn(self, X_train, y_train, epochs=50, batch_size=32, verbose=2):
        """
        train the model
        """
    ...
    def retrain_nn(self, model, X_train, X_test, y_train, y_test, retrain_period=30):
        ...
        self.model = model
        self.train_cnn(X_train, y_train, verbose=0)
        ...
        for i in range(dim):
            ...
            X_train = np.append(X_train, X_test[0:1], axis=0)
            X_test = X_test[1:]
            y_train = np.append(y_train, y_test[0:1], axis=0)
            y_test = y_test[1:]
