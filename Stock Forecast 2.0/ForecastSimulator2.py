...
def forecast_split_classifier(data, window_size=30, dtp=1, test_days=30):
    """
    attributes:
        data = time series data
        window_size = time window to reshape data, after the reshape the data will have dim (data.shape[0] - window_size, window_size)
    ...
    for i in range(window_size, data.shape[0]-1-dtp):
        X.append(data.values[i+1-window_size:i+1,0].tolist())
        target = (data.values[i+1:i+1+dtp,0] - data.values[i:i+dtp,0])
        if target >= 0:
            y_true.append([1])
        else:
            y_true.append([0])
            
        y_val.append(data.values[i+1:i+1+dtp,0])
    ...
    X_train = X[:-test_days,:]
    X_test = X[-test_days:,:]
    y_train = y_true[:-test_days]
    y_test = y_true[-test_days:]
    y_val = y_val[-test_days:]
        
    return X_train, X_test, y_train, y_test, y_val
...
class Forecastsimulator():
    ...
    def retrain_classifier(self, model, X_train, X_test, y_train, y_test, retrain_period=30):
        ...
        self.model = model
        self.train(X_train, y_train)
        prediction = np.empty(shape=(1,len(y_test)))
        dim = X_test.shape[0]
    ...
    def simulate_classifier(self, prediction, y_test, y_val):
        ...
        pred_diff = prediction.tolist()
        true_diff = []

        for i in range(y_test.shape[0]-1):
            true_diff.append(y_val[i+1,0] - y_val[i,0])
        ...
        for i in range(1, y_val.shape[0]-1):
            ...
            percentage = (abs(y_val[i,0] - y_val[i-1,0])/y_val[i-1,0])
